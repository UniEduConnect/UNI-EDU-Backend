...
- **Mapping**: **Mapster** is the preferred mapper for new code — call `source.Adapt<TDest>()` (or `source.Adapt(existing)` to project onto an existing instance) directly in services. No `IMapper` injection or profile registration is required for the default convention-based maps; add a `TypeAdapterConfig` only when you need custom member rules. AutoMapper is still registered (`AddAutoMapper(typeof(MappingProfile))` in `Program.cs`) and used by legacy code such as [`AuthService`](UNI-EDU-Backend.Application/Services/Auths/AuthService.cs); leave existing usages in place, but do not add new `CreateMap` entries or inject `IMapper` for new services.
